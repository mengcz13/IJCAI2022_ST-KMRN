...
commands = [
    r'python train_multi_step.py --data {root_path} --seq_in_len {seq_len} --seq_out_len {pred_len} --seq_diff {seq_diff}{chrono_emb} --test_seq_diff {test_seq_diff} --keep_ratio {keep_ratio} --batch_size {batch_size} --epochs {epochs} --target_res {target_res} --resolution_type {resolution_type} --expname_suffix "res-{target_res}-{resolution_type}" --patience {patience} --seed {seed}', # all resolutions at the same time
