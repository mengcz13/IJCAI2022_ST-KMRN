...
class mtgnn_enc(nn.Module):
    def __init__(self, gcn_true, buildA_true, gcn_depth, num_nodes, device, predefined_A=None, static_feat=None, 
        ...
        skip_channels=64, end_channels=128, seq_length=12, in_dim=2, out_dim=12, layers=3, propalpha=0.05, 
        tanhalpha=3, layer_norm_affline=True):
        super(mtgnn_enc, self).__init__()
    ...
    def forward(self, input, idx=None):
        # input: [bs, T, N, F]
        input = input.transpose(1, 3) # [B, F, N, T]
        ...
        x = F.relu(self.end_conv_2(x)) # [bs, end_channel, node, 1]
        x = x.squeeze(-1).permute(0, 2, 1) # [bs, node, end_channel]

        # # [bs, output_len * fnum, node, 1] -> [bs, fnum, node, output_len]
        # x = x[:, :, :, 0]
        # bs, noden = x.shape[0], x.shape[2]
        # x = x.reshape(bs, self.args.seq_out_len,
        #               self.args.out_dim, noden).permute(0, 2, 3, 1)

        return x


class mtgnn_dec(nn.Module):
