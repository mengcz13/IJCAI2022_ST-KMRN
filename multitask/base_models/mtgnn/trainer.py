...
class Trainer():
    ...
    def train(self, input, real_val, idx=None):
        ...
        # if self.cl:
        #     loss = self.loss(
        #         predict[:, :self.task_level, :, :], real[:, :self.task_level, :, :], 0.0)
        # else:
        #     loss = self.loss(predict, real, 0.0)

        loss = 0
        for pr, rr in zip(predict, real):
            loss += self.loss(pr, rr, 0.0)
    ...
    def eval(self, input, real_val):
        ...
        predict = output
        real = real_val
        loss = sum([self.loss(pr, rr, 0.0) for pr, rr in zip(predict, real)])
        mape = sum([util.masked_mape(pr, rr, 0.0).item() for pr, rr in zip(predict, real)])
        rmse = sum([util.masked_rmse(pr, rr, 0.0).item() for pr, rr in zip(predict, real)])
        return loss.item(), mape, rmse
    ...
    def _makeOptimizer(self):
        if self.method == 'sgd':
            self.optimizer = optim.SGD(
                self.params, lr=self.lr, weight_decay=self.lr_decay)
