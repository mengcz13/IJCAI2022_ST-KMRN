...
class StandardScaler():
    ...
    def __init__(self, mean, std):
        self.mean = mean
        self.std = std
        self.mean_ts = torch.FloatTensor(self.mean)
        self.std_ts = torch.FloatTensor(self.std)

    def transform(self, data):
        if type(data) is torch.Tensor:
            return (data - self.mean_ts.to(data.device)) / self.std_ts.to(data.device)
        else:
            return (data - self.mean) / self.std

    def inverse_transform(self, data):
        if type(data) is torch.Tensor:
            return (data * self.std_ts.to(data.device)) + self.mean_ts.to(data.device)
        else:
